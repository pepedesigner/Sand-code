...
- **Absent means signed out.** The bootstrap redirects to `signin.html` whenever the key is missing — including when `localStorage` throws (private mode), because a browser that refuses storage must not be able to open the console. It sets `auth-pending` first, which hides the page, so a slow redirect never flashes the console.
