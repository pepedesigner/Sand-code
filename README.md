...
Both auth pages sit on a field of dots that tints toward the accent under the pointer, scatters when the pointer moves fast, ripples out from a click and springs back. It is the React Bits `<DotGrid />` idea ported to plain canvas (`initAuthDots` in `script.js`): the reference hands the return to GSAP's `InertiaPlugin`, and a damped spring does the same job here, so the site stays dependency-free. Colours are read from `--muted` and `--accent` — the same pair the hero grid uses — so the two fields match and both follow a theme switch. Under `prefers-reduced-motion` the field is still drawn, it simply does not react.

Unlike the hero's grid this one *moves*, so it cannot keep a static base layer with lit dots drawn over it — a displaced dot would leave its original behind. The whole field is redrawn instead, and the loop only runs while something is actually happening: a pointer move or a click starts it, and it parks the moment every dot is home.
